...
SELECT
  employee_id,
  first_name || ' ' || last_name AS full_name,
  salary AS monthly_salary_base,
  (salary * 12) AS annual_salary,
  ROUND((salary * 12) / 12, 2) AS monthly_salary_rounded,
  ROUND((salary * 0.10) * 12, 2) AS annual_10pct_raise_amount
FROM employees;
...
SELECT
  project_id,
  format('Project: %s - Budget: $%s - Status: %s', project_name, budget::text, status) AS project_label
FROM projects;
...
SELECT
  ...
  e.first_name || ' ' || e.last_name AS full_name,
  e.department,
  ROUND(AVG(a.hours_worked) OVER (PARTITION BY e.employee_id), 2) AS avg_hours_worked,
...
SELECT
  ...
  GREATEST(ds.avg_salary, 50000) AS greatest_avg_vs_50000,
  LEAST(ds.avg_salary, 50000) AS least_avg_vs_50000
FROM dept_stats ds
LEFT JOIN employees e ON e.salary = ds.max_salary AND e.department = ds.department;
